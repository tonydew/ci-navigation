...
class Navigation
{
	...
	public function build($url, $title = NULL, $params = array())
	{
		...
		if (is_array($url))
		{
			...
			foreach ($url as $u)
			{
				$u['params']          = (isset($u['params'])) ? $u['params'] : array();
				$u['children']        = (isset($u['children'])) ? $u['children'] : array();

				// invoke the builder  
				$built[] = $this->_build($u['url'], $u['title'], $u['params'], $u['children']);
			}
			
			// implode the navigation to create a string of li's to be used in html 
			$this->nav_elements = implode("\n", $built);
		}
		else
		{
